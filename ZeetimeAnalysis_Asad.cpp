...
void HistSet::book(TFileDirectory subDir, const std::string& post) {
  ...
  eta1_ =(TH1F*) subDir.make<TH1F>("eta1","eta1",100, eta1_max, eta1_min);
  phi1_ =(TH1F*) subDir.make<TH1F>("phi1","phi1",100,phi1_max,phi1_min);
  eta2_ =(TH1F*) subDir.make<TH1F>("eta2","eta2",100, eta2_max, eta2_min);
  phi2_ =(TH1F*) subDir.make<TH1F>("phi2","phi2",100,phi2_max,phi2_min);
  e1pt_ =(TH1F*) subDir.make<TH1F>("e1pt","e1pt", 120, 0, 120);
  e2pt_ =(TH1F*) subDir.make<TH1F>("e2pt","e2pt", 120, 0, 120);
  e1E_ =(TH1F*) subDir.make<TH1F>("e1E","e1E", 300, 0, 300);
  e2E_ =(TH1F*) subDir.make<TH1F>("e2E","e2E", 300, 0, 300);
...
void HistSet::fill(int sc1, int sc2, int bc1, int bc2 ){
  ...
  eta1_		->Fill(treeVars_.superClusterEta[sc1]);
  eta2_		->Fill(treeVars_.superClusterEta[sc2]);
  phi1_		->Fill(treeVars_.superClusterPhi[sc1]);
  phi2_		->Fill(treeVars_.superClusterPhi[sc2]);
  e1pt_ 	->Fill(et1);
  e2pt_ 	->Fill(et2);
  e1E_ 		->Fill(treeVars_.superClusterRawEnergy[sc1]);
  e2E_ 		->Fill(treeVars_.superClusterRawEnergy[sc2]);
...
bool Within_Eta_Phi( int sc, int eta_min, int phi_min, int eta_max, int phi_max )
{
	bool event_selected = true;

	float eta = treeVars_.superClusterEta[sc];
		if (eta < eta_min || eta > eta_max) event_selected = false;  //... CHECK IF ETA IS WITHIN THE SPECIFIED RANGE ...\\
	
	float phi = treeVars_.superClusterEta[sc];
		if (phi < phi_min || phi > phi_max) event_selected = false; //... CHECK IF PHI IS WITHIN THE SPECIFIED RANGE ...\\
...
int main (int argc, char** argv)
{
  ...
  for (int entry = 0 ; (entry < nEntries && eventCounter < numEvents_); ++entry)
  {
    ...
    for (int sc1=0; sc1<treeVars_.nSuperClusters; sc1++){
      ...
      if (! Within_Eta_Phi(sc1, eta1_min, phi1_min, eta1_max, phi1_max) ) continue;
	...
	if (! Within_Eta_Phi(sc2, eta2_min, phi2_min, eta2_max, phi2_max) ) continue;
